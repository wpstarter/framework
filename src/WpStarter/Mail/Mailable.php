...
use WpStarter\Contracts\Filesystem\Factory as FilesystemFactory;
use WpStarter\Contracts\Mail\Factory as MailFactory;
use WpStarter\Contracts\Mail\Mailable as MailableContract;
use WpStarter\Contracts\Queue\Factory as Queue;
...
use WpStarter\Support\Traits\Localizable;
...
class Mailable implements MailableContract, Renderable
{
    ...
    protected function hasRecipient($address, $name = null, $property = 'to')
    {
        ...
        return ws_collect($this->{$property})->contains(function ($actual) use ($expected) {
    ...
    {
        $this->attachments = ws_collect($this->attachments)
    ...
    {
        $this->diskAttachments = ws_collect($this->diskAttachments)->push([
            'disk' => $disk,
            'path' => $path,
    ...
    {
        $this->rawAttachments = ws_collect($this->rawAttachments)
