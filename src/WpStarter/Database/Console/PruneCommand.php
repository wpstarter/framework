...
class PruneCommand extends Command
{
    ...
    protected function models()
    {
        if (! empty($models = $this->option('model'))) {
            return ws_collect($models)->filter(function ($model) {
        ...
        return ws_collect((new Finder)->in($this->getDefaultPath())->files()->name('*.php'))
            ->map(function ($model) {
    ...
    {
        $uses = ws_class_uses_recursive($model);
    ...
    protected function pretendToPrune($model)
    {
        ...
        $count = $instance->prunable()
            ->when(in_array(SoftDeletes::class, ws_class_uses_recursive(get_class($instance))), function ($query) {
                $query->withTrashed();
            })->count();
