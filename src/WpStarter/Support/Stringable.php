...
class Stringable implements JsonSerializable
{
    ...
    {
        return ws_collect(explode($delimiter, $this->value, $limit));
    }
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, FILTER_VALIDATE_INT) !== false) {
            return ws_collect(mb_str_split($this->value, $pattern));
        }

        $segments = preg_split($pattern, $this->value, $limit, $flags);

        return ! empty($segments) ? ws_collect($segments) : ws_collect();
    }
    ...
    {
        return ws_collect(sscanf($this->value, $format));
    }
    ...
    {
        return ws_collect(Str::ucsplit($this->value));
    }
