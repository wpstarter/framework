...
class BusFake implements QueueingDispatcher
{
     ...
     *
     * @var \WpStarter\Contracts\Bus\QueueingDispatcher
     */
     ...
     *
     * @param  \WpStarter\Contracts\Bus\QueueingDispatcher  $dispatcher
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            ws_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return ws_tap(clone $job, function ($job) {
    ...
    {
        $chain = ws_collect($expectedChain)->map(function ($job) {
    ...
    {
        return ! ws_collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return ws_collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return ws_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return ws_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return ws_collect($this->batches)->filter(function ($batch) use ($callback) {
     ...
     *
     * @param  \WpStarter\Support\Collection|array  $jobs
     ...
     *
     * @param  \WpStarter\Support\Collection|array  $jobs
     * @return \WpStarter\Bus\PendingBatch
     */
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return ws_collect($this->jobsToFake)
