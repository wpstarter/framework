...
use Closure;
use WpStarter\Contracts\Mail\Factory;
use WpStarter\Contracts\Mail\Mailable;
use WpStarter\Contracts\Mail\Mailer;
use WpStarter\Contracts\Mail\MailQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = ws_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = ws_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return ws_collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return ws_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }
     ...
     *
     * @param  mixed  $users
     * @return \WpStarter\Mail\PendingMail
     */
