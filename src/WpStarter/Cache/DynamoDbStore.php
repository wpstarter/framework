...
class DynamoDbStore implements LockProvider, Store
{
    ...
    public function many(array $keys)
    {
        ...
        $response = $this->dynamo->batchGetItem([
            'RequestItems' => [
                $this->table => [
                    'ConsistentRead' => false,
                    'Keys' => ws_collect($prefixedKeys)->map(function ($key) {
                        return [
                            $this->keyAttribute => [
                                'S' => $key,
                            ],
        ...
        return array_merge(ws_collect(array_flip($keys))->map(function () {
            //
        })->all(), ws_collect($response['Responses'][$this->table])->mapWithKeys(function ($response) use ($now) {
    ...
    public function putMany(array $values, $seconds)
    {
        ...
        $this->dynamo->batchWriteItem([
            'RequestItems' => [
                $this->table => ws_collect($values)->map(function ($value, $key) use ($expiration) {
                    return [
                        'PutRequest' => [
                            'Item' => [
                                $this->keyAttribute => [
