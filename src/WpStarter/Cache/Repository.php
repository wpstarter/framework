...
use WpStarter\Cache\Events\CacheHit;
use WpStarter\Cache\Events\CacheMissed;
use WpStarter\Cache\Events\KeyForgotten;
use WpStarter\Cache\Events\KeyWritten;
use WpStarter\Contracts\Cache\Repository as CacheContract;
...
/**
 * @mixin \WpStarter\Contracts\Cache\Store
 */
class Repository implements ArrayAccess, CacheContract
{
     ...
     *
     * @var \WpStarter\Contracts\Cache\Store
     */
    protected $store;
    ...
    {
        $values = $this->store->many(ws_collect($keys)->map(function ($value, $key) {
            return is_string($key) ? $key : $value;
        })->values()->all());

        return ws_collect($values)->map(function ($value, $key) use ($keys) {
    ...
    protected function handleManyResult($keys, $key, $value)
    {
        ...
        if (is_null($value)) {
            ...
            return isset($keys[$key]) ? ws_value($keys[$key]) : null;
        }
    ...
    public function remember($key, $ttl, Closure $callback)
    {
        ...
        $this->put($key, $value = $callback(), ws_value($ttl));
     ...
     * @return \WpStarter\Cache\TaggedCache
     *
     ...
     *
     * @return \WpStarter\Contracts\Cache\Store
     */
