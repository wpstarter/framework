...
use ArrayAccess;
use Countable;
use WpStarter\Contracts\Pagination\LengthAwarePaginator as LengthAwarePaginatorContract;
...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, JsonSerializable, LengthAwarePaginatorContract
{
     ...
     * @param  array  $data
     * @return \WpStarter\Contracts\Support\Htmlable
     */
    ...
    {
        return ws_collect($this->elements())->flatMap(function ($item) {
            if (! is_array($item)) {
                return [['url' => null, 'label' => '...', 'active' => false]];
            }

            return ws_collect($item)->map(function ($url, $page) {
                return [
                    'url' => $url,
                    'label' => (string) $page,
