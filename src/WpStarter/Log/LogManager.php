...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \WpStarter\Log\Logger  $logger
     * @return \WpStarter\Log\Logger
     */
    ...
    protected function createStackDriver(array $config)
    {
        ...
        $handlers = ws_collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
                ? $channel->getHandlers()
                : $this->channel($channel)->getHandlers();
        })->all();

        $processors = ws_collect($config['channels'])->flatMap(function ($channel) {
            return $channel instanceof LoggerInterface
    ...
    {
        return ws_tap(new LineFormatter(null, $this->dateFormat, true, true), function ($formatter) {
