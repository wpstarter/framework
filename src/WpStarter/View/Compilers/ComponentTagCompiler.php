...
use WpStarter\View\DynamicComponent;
use WpStarter\View\ViewFinderInterface;
...
class ComponentTagCompiler
{
     ...
     * @param  \WpStarter\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
    ...
    public function partitionDataAndAttributes($class, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            return [ws_collect($attributes), ws_collect($attributes)];
        }

        $constructor = (new ReflectionClass($class))->getConstructor();

        $parameterNames = $constructor
                    ? ws_collect($constructor->getParameters())->map->getName()->all()
                    : [];

        return ws_collect($attributes)->partition(function ($value, $key) use ($parameterNames) {
    ...
    protected function getAttributesFromAttributeString(string $attributeString)
    {
        ...
        return ws_collect($matches)->mapWithKeys(function ($match) {
    ...
    {
        return ws_collect(token_get_all($value))->map(function ($token) {
            if (! is_array($token)) {
                return $token;
            }
