...
class ContainerCallTest extends TestCase
{
    ...
    {
        $container = new Container;
        $result = $container->call('WpStarter\Tests\Container\ContainerStaticMethodStub::inject');
        $this->assertInstanceOf(ContainerCallConcreteStub::class, $result[0]);
        $this->assertSame('taylor', $result[1]);
    }
    ...
    {
        $container = new Container;
        $result = $container->call('WpStarter\Tests\Container\containerTestInject');
        $this->assertInstanceOf(ContainerCallConcreteStub::class, $result[0]);
        $this->assertSame('taylor', $result[1]);
    }
    ...
    public function testCallWithoutRequiredParamsThrowsException()
    {
        ...
        $this->expectExceptionMessage('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class WpStarter\Tests\Container\ContainerTestCallStub');
    ...
    {
        $this->expectException(BindingResolutionException::class);
        $this->expectExceptionMessage('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class WpStarter\Tests\Container\ContainerCallTest');
