...
class CommandSchedulingTestCommand_{$this->id} extends WpStarter\Console\Command
{
    ...
    {
        \$logfile = {$logfile};
        (new WpStarter\Filesystem\Filesystem)->append(\$logfile, "handled\\n");
    }
}

// Register command with Kernel
WpStarter\Console\Application::starting(function (\$artisan) {
...
WpStarter\Foundation\Application::getInstance()
    ->booted(function (\$app) {
        \$app->resolving(WpStarter\Console\Scheduling\Schedule::class, function(\$schedule) {
            \$fs = new WpStarter\Filesystem\Filesystem;
