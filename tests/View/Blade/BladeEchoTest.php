...
class BladeEchoTest extends AbstractBladeTestCase
{
    public function testEchosAreCompiled()
    {
        ...
        $this->assertSame('<?php echo ws_e($name); ?>', $this->compiler->compileString('{{{$name}}}'));
        $this->assertSame('<?php echo ws_e($name); ?>', $this->compiler->compileString('{{$name}}'));
        $this->assertSame('<?php echo ws_e($name); ?>', $this->compiler->compileString('{{ $name }}'));
        $this->assertSame('<?php echo ws_e($name); ?>', $this->compiler->compileString('{{
            $name
        }}'));
        $this->assertSame("<?php echo ws_e(\$name); ?>\n\n", $this->compiler->compileString("{{ \$name }}\n"));
        $this->assertSame("<?php echo ws_e(\$name); ?>\r\n\r\n", $this->compiler->compileString("{{ \$name }}\r\n"));
        $this->assertSame("<?php echo ws_e(\$name); ?>\n\n", $this->compiler->compileString("{{ \$name }}\n"));
        $this->assertSame("<?php echo ws_e(\$name); ?>\r\n\r\n", $this->compiler->compileString("{{ \$name }}\r\n"));

        $this->assertSame('<?php echo ws_e("Hello world or foo"); ?>',
            $this->compiler->compileString('{{ "Hello world or foo" }}'));
        $this->assertSame('<?php echo ws_e("Hello world or foo"); ?>',
            $this->compiler->compileString('{{"Hello world or foo"}}'));
        $this->assertSame('<?php echo ws_e($foo + $or + $baz); ?>', $this->compiler->compileString('{{$foo + $or + $baz}}'));
        $this->assertSame('<?php echo ws_e("Hello world or foo"); ?>', $this->compiler->compileString('{{
            "Hello world or foo"
        }}'));
        ...
        $this->assertSame('<?php echo ws_e(myfunc(\'foo or bar\')); ?>',
            $this->compiler->compileString('{{ myfunc(\'foo or bar\') }}'));
        $this->assertSame('<?php echo ws_e(myfunc("foo or bar")); ?>',
            $this->compiler->compileString('{{ myfunc("foo or bar") }}'));
        $this->assertSame('<?php echo ws_e(myfunc("$name or \'foo\'")); ?>',
