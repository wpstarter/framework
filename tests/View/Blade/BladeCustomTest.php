...
<?php endif; ?>';
    ...
    public function testCustomConditions()
    {
        ...
        $string = '@custom($user)
@endcustom';
        $expected = '<?php if (\WpStarter\Support\Facades\Blade::check(\'custom\', $user)): ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
...
@endcustom';
        $expected = '<?php if (\WpStarter\Support\Facades\Blade::check(\'custom\', $user)): ?>
<?php elseif (\WpStarter\Support\Facades\Blade::check(\'custom\', $product)): ?>
<?php else: ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
...
@elsecustom(0)
@endcustom';
        $expected = '<?php if (\WpStarter\Support\Facades\Blade::check(\'custom\', 0)): ?>
<?php elseif (\WpStarter\Support\Facades\Blade::check(\'custom\', 0)): ?>
<?php endif; ?>';
        $this->assertEquals($expected, $this->compiler->compileString($string));
    }
